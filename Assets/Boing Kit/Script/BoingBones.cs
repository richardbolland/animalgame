...
namespace BoingKit
{
  public class BoingBones : BoingReactor
  {
    ...
    private class RescanEntry
    {
      ...
      internal RescanEntry(Transform transform, Transform parentTransform, int iParent, float lengthFromRoot)
      {
        Transform = transform;
    ...
    public void RescanBoneChains()
    {
      ...
      for (int iChain = 0; iChain < numChains; ++iChain)
      {
        ...
        {
#if UNITY_EDITOR
          Debug.LogWarning($"Bone chain {iChain} has null root.", this);
#endif
        ...
        boneQueue.Enqueue(new RescanEntry(root, null, -1, 0.0f));
        while (boneQueue.Count > 0)
        {
          ...
          for (int iChild = 0, numChildren = boneTransform.childCount; iChild < numChildren; ++iChild)
          {
            ...
            {
#if UNITY_EDITOR
              Debug.LogWarning($"Bone chain {iChain} has null descendant (parent: {entry.ParentTransform}, child index: {iChild}.", this);
#endif
              continue;
            }
            ...
            boneQueue.Enqueue(new RescanEntry(childTransform, boneTransform, iBone, lengthFromRoot));
    ...
    public void Reboot(int iChain)
    {
      ...
      for (int iBone = 0; iBone < aBone.Length; ++iBone)
      {
        ...
        bone.Instance.ScaleSpring.Reset(bone.Transform.localScale);
    ...
    public override void HandleTeleport()
    {
      base.HandleTeleport();
    ...
    public void HandleTeleport(int iChain)
    {
      var aBone = BoneData[iChain];
      ...
      {
        var bone = aBone[iBone];
        bone.Instance.PositionSpring.Value = bone.Transform.position;
        bone.Instance.RotationSpring.ValueQuat = bone.Transform.rotation;
        bone.Instance.ScaleSpring.Value = bone.Transform.localScale;
        bone.CachedPositionWs = bone.Transform.position;
        bone.CachedPositionLs = bone.Transform.localPosition;
        bone.CachedRotationWs = bone.Transform.rotation;
        bone.CachedRotationLs = bone.Transform.localRotation;
